...
def mock_gcs(func):
    ...
    def inner(*args, **kwargs):
        with mock.patch(
            ...
            try:  # is it a method that needs a self arg and mock args?
                self_arg = inspect.signature(func).self
                func(self_arg, mocked_client, mocked_session, *args, **kwargs)
...
class SeekableBufferedInputBaseTest(unittest.TestCase):
    ...
    @maybe_mock_gcs
    def test_iter_context_manager(self, *args):
    ...
    @maybe_mock_gcs
    def test_seek_beginning(self, *args):
    ...
    @maybe_mock_gcs
    def test_seek_current(self, *args):
    ...
    @maybe_mock_gcs
    def test_readline_tiny_buffer(self, *args):
        content = b'englishman\nin\nnew\nyork\n'
        put_to_bucket(contents=content)
    ...
    @maybe_mock_gcs
    def test_read0_does_not_return_data(self, *args):
        content = b'englishman\nin\nnew\nyork\n'
        put_to_bucket(contents=content)
...
class MakeRangeStringTest(unittest.TestCase):
    def test_no_stop(self):
        ...
        self.assertEqual(smart_open.gcs.make_range_string(start, stop), 'bytes 1-/*')

    def test_stop(self):
        start, stop, end = 1, 2, smart_open.gcs._UNKNOWN_FILE_SIZE
        self.assertEqual(smart_open.gcs.make_range_string(start, stop), 'bytes 1-2/*')
