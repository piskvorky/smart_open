...
class BufferedOutputBase(io.BufferedIOBase):
    def __init__(self, uri_path, min_part_size=WEBHDFS_MIN_PART_SIZE):
        ...
        init_response = requests.put("http://" + self.uri_path, params=payload, allow_redirects=False)
    ...
    def _upload(self, data):
        payload = {"op": "APPEND"}
        init_response = requests.post("http://" + self.uri_path, params=payload, allow_redirects=False)
        if not init_response.status_code == httplib.TEMPORARY_REDIRECT:
            raise WebHdfsException(str(init_response.status_code) + "\n" + init_response.content)
        uri = init_response.headers['location']
    ...
    def write(self, b):
        ...
        if self.chunk_bytes >= self.min_part_size:
            ...
            logger.info(
                "uploading part #%i, %i bytes (total %.3fGB)", self.parts, len(buff), self.total_size / 1024.0 ** 3
            )
            self._upload(buff)
    ...
    def close(self):
        ...
        if buff:
            logger.info(
                "uploading last part #%i, %i bytes (total %.3fGB)", self.parts, len(buff), self.total_size / 1024.0 ** 3
            )
            self._upload(buff)
